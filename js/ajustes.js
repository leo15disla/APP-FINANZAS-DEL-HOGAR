...
export function initAjustes({ notify, onChange }) {
  ...
  const temaToggle = document.getElementById('toggle-tema');
  const temaEstado = document.getElementById('tema-estado');
  const btnTemaAuto = document.getElementById('btn-tema-auto');
  ...
  const actualizarTema = (preferencia) => {
    if (!state.settings) state.settings = { sonido: true, tema: 'auto' };
    state.settings.tema = preferencia;
    applyTheme(preferencia);
    if (temaEstado) {
      const resolved = resolveTheme(preferencia);
      temaEstado.textContent =
        preferencia === 'auto'
          ? 'Se adapta automáticamente a tu sistema.'
          : resolved === 'dark'
          ? 'Modo oscuro activo.'
          : 'Modo claro activo.';
    }
    if (temaToggle) {
      temaToggle.checked = resolveTheme(preferencia) === 'dark';
    }
  };

  const preferenciaInicial = state.settings?.tema || 'auto';
  if (temaToggle) {
    temaToggle.checked = resolveTheme(preferenciaInicial) === 'dark';
    temaToggle.addEventListener('change', () => actualizarTema(temaToggle.checked ? 'dark' : 'light'));
  }
  btnTemaAuto?.addEventListener('click', () => actualizarTema('auto'));
  actualizarTema(preferenciaInicial);
