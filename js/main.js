...
import { initAjustes, cargarDatosPrueba } from './ajustes.js';
...
function setupRouter() {
  const secciones = {
    ...
    ajustes: document.getElementById('seccion-ajustes')
  };
  const botones = {
...
function initApp() {
  applyTheme(state.settings?.tema || 'auto');
  mostrarSplash();
  const media = window.matchMedia?.('(prefers-color-scheme: dark)');
  media?.addEventListener('change', () => {
    if ((state.settings?.tema || 'auto') === 'auto') applyTheme('auto');
  });
  setupRouter();
  initCuentas({ notify });
  ...
  initDashboard();
  initAjustes({ notify, onChange: onDataChange });
...
function exportarCSV() {
  ...
  downloadFile('movimientos-mes.csv', csv, 'text/csv');
}

function exportarJSON() {
  const movimientos = movimientosMesActual();
  downloadFile('movimientos-mes.json', JSON.stringify(movimientos, null, 2), 'application/json');
}
...
function exportarPDFMes() {
  ...
  downloadFile('movimientos-mes.pdf', blob, 'application/pdf');
...
function exportarPDFHistorial() {
  ...
  downloadFile(`historial-${mesSeleccionado || 'mes'}.pdf`, blob, 'application/pdf');
...
function exportarPDFDashboard() {
  ...
  downloadFile('dashboard-resumen.pdf', blob, 'application/pdf');
...
function exportarXLSX() {
  ...
  downloadFile('finanzas_mes.xlsx', blob, blob.type);
...
async function compartirWhatsApp() {
  ...
  downloadFile(ultimoArchivo.nombre, ultimoArchivo.blob, ultimoArchivo.blob.type);
  const hint = document.getElementById('share-hint');
...
function initOnboarding() {
  ...
  const dots = Array.from(document.querySelectorAll('#onboarding-overlay .onboarding-dots span'));
  ...
  const hasData =
    (state.movimientos && state.movimientos.length > 0) ||
    (state.prestamos && state.prestamos.length > 0) ||
    (state.historial && Object.keys(state.historial || {}).length > 0);
  if (localStorage.getItem(key) === '1' || hasData) {
    overlay.remove();
    return;
  }
  ...
  const slides = [
    {
      title: 'Organiza tus gastos e ingresos fácilmente',
      desc: 'Registra movimientos, clasifícalos y mira tus KPIs en segundos.'
    },
    {
      title: 'Controla préstamos, cuentas y presupuesto 50/30/20',
      desc: 'Mantén tarjetas, préstamos y cuentas bancarias bajo una sola vista.'
    },
    {
      title: 'Tus datos se guardan localmente',
      desc: 'La información vive en este dispositivo y puedes exportarla cuando quieras.'
    }
  ];
  let index = 0;
  ...
  const render = () => {
    ...
    if (title) title.textContent = actual.title;
    if (text) text.textContent = actual.desc;
    dots.forEach((d, i) => d.classList.toggle('active', i === index));
    if (btnNext) btnNext.textContent = index === slides.length - 1 ? 'Empezar' : 'Continuar';
  };
