...
function categoriasDisponibles() {
  const personalizadas = (state.presupuesto.categorias || []).map((c) => ({
    nombre: c.nombre,
    icono: c.icono || '✨',
    keywords: [],
    color: c.color,
  ...
  return [...categoriasBase, ...personalizadas];
}
...
function buscarCategoria(nombre) {
  return categoriasDisponibles().find((c) => c.nombre === nombre);
}

function iconoCategoria(nombre) {
  const cat = buscarCategoria(nombre);
  return `${cat?.icono || categoriaIconos[nombre] || '🏷️'} ${nombre}`;
}
...
function poblarCategoriasSelectores() {
  ...
  if (selectCategoria) {
    selectCategoria.innerHTML = lista
      .map((c) => `<option value="${c.nombre}">${c.icono} ${c.nombre}</option>`)
      .join('');
  }
  if (filtroCat) {
    filtroCat.innerHTML = ['<option value="Todas">Todas las categorías</option>',
      ...lista.map((c) => `<option value="${c.nombre}">${c.icono} ${c.nombre}</option>`)];
  }
  if (editCategoria) {
    editCategoria.innerHTML = lista
      .map((c) => `<option value="${c.nombre}">${c.icono} ${c.nombre}</option>`)
      .join('');
  }
}
...
export function initMovimientos({ notify, onChange }) {
  ...
  window.addEventListener('categorias-actualizadas', () => {
    poblarCategoriasSelectores();
    poblarSugerencias(inputs.categoria.value);
    renderMovimientos();
  });
